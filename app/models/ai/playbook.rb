...
class Ai::Playbook < ApplicationRecord
  ...
  # AGENTE (department.agent.handoff_team_ids). Antes só a UI gateava (select populado da whitelist); um save
  ...
  def on_complete_teams_in_whitelist
    whitelist = Array(department&.agent&.handoff_team_ids).map(&:to_i)
