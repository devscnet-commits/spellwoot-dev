...
<template>
  <div class="p-6 col-span-full w-full max-w-3xl flex flex-col gap-6">
    ...
    <template v-else>
      ...
      <div
        v-for="group in [
          { key: 'won', list: wonReasons, label: $t('OPERATIONAL_FLOWS_SETTINGS.FORM.WON_REASONS') },
          { key: 'lost', list: lostReasons, label: $t('OPERATIONAL_FLOWS_SETTINGS.FORM.LOST_REASONS') },
        ]"
        :key="group.key"
        class="flex flex-col gap-2"
      >
        <label class="text-sm font-medium text-n-slate-12">{{ group.label }}</label>
        <div
        ...
        >
          <input
            ...
            type="text"
            :placeholder="$t('OPERATIONAL_FLOWS_SETTINGS.FORM.REASON_PLACEHOLDER')"
      ...
      <div class="flex justify-end">
        <Button
          ...
          :disabled="!name.trim() || isSaving || uiFlags.isCreating || uiFlags.isUpdating"
          :is-loading="isSaving"
          @click="save"
        />
      </div>
