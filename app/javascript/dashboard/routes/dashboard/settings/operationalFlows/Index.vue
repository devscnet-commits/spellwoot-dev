...
import { useStore, useMapGetter, useStoreGetters } from 'dashboard/composables/store';
...
<template>
  <SettingsLayout
    ...
    :loading-message="$t('OPERATIONAL_FLOWS_SETTINGS.LOADING')"
    :no-records-found="!flowsList.length"
    :no-records-message="$t('OPERATIONAL_FLOWS_SETTINGS.LIST.404')"
  >
    <template #header>
      <BaseSettingsHeader
        :title="$t('OPERATIONAL_FLOWS_SETTINGS.HEADER')"
        :description="$t('OPERATIONAL_FLOWS_SETTINGS.DESCRIPTION')"
      >
        <template #actions>
          <router-link
            v-if="isAdmin"
            ...
            <div class="flex flex-col items-start gap-1">
              ...
              <div class="flex items-center gap-3 mt-1">
                <span class="flex items-center gap-1 text-xs text-n-slate-11">
                  ...
                  {{ $t('OPERATIONAL_FLOWS_SETTINGS.LIST.REASONS_COUNT', { count: reasonsCount(flow) }) }}
                </span>
                <span
