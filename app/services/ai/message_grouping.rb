# Debounce/grouping for inbound customer messages. When an agent sets a grouping delay, the
...
module Ai
  module MessageGrouping
    ...
    # Gateway in additional_attributes['ai_step']); otherwise the agent's general delay.
    # nil (unset) = falls back to the general delay. 0 (explicit) = no grouping on THIS step, even
    # when the agent has a general delay — so a step can opt out of grouping on purpose.
    ...
    # Max general grouping delay across the agents attending this inbox.
    def general_delay(inbox_id)
      ...
      Ai::Agent.where(id: agent_ids)
               .map { |agent| agent.behavior.to_h.dig('grouping', 'delay_seconds').to_i }
               .push(0).max
    end
