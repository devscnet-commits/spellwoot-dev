...
RSpec.describe Ai::FollowupConversationJob do
  ...
  describe '#run — resolução de binding com MÚLTIPLOS agentes live na mesma inbox' do
    ...
    it 'conversa JÁ atendida: usa o agente REAL do Ai::Run, não o outro agente também live na inbox' do
      agent_com_config = Ai::Agent.create!(
        account: account, name: 'Maya v5.0', status: 'active', ai_operation_profile_id: profile.id,
      ...
      agent_sem_config = Ai::Agent.create!(account: account, name: 'Maya v6.0', status: 'active',
                                           ai_operation_profile_id: profile.id, behavior: {}, follow_up: {})
    ...
    it 'conversa NOVA (sem Ai::Run ainda): elege por [priority, id] entre os live elegíveis, igual ao Gateway' do
      agent_priority_2 = Ai::Agent.create!(account: account, name: 'Prioridade 2', status: 'active',
                                           ai_operation_profile_id: profile.id, behavior: {}, follow_up: {})
      agent_priority_1 = Ai::Agent.create!(
        account: account, name: 'Prioridade 1', status: 'active', ai_operation_profile_id: profile.id,
        behavior: { 'auto_attendance' => true, 'reply_scope' => 'all' },
        follow_up: { 'behaviors' => [{ 'context' => 'inbox_hours', 'attempts' => [{ 'message' => 'vamos seguir?', 'delay_minutes' => 1 }] }] }
      )
  ...
  describe '#inactivity_minutes' do
    it 'falls back to the default when unset' do
      expect(job.send(:inactivity_minutes, instance_double(Ai::Agent, close_rules: {}))).to eq(30)
    end

    it 'uses the configured value' do
      agent = instance_double(Ai::Agent, close_rules: { 'inactivity_minutes' => 15 })
      expect(job.send(:inactivity_minutes, agent)).to eq(15)
    end
  end
  ...
  describe '#fallback_actions' do
    it 'reads the ordered no_followup_actions list (order = priority)' do
      ...
      expect(job.send(:fallback_actions, agent)).to eq(%w[transfer_human finalize])
    end

    it 'drops blanks and is empty when unset' do
      agent = instance_double(Ai::Agent, close_rules: { 'no_followup_actions' => ['', 'wait'] })
      expect(job.send(:fallback_actions, agent)).to eq(['wait'])
      expect(job.send(:fallback_actions, instance_double(Ai::Agent, close_rules: {}))).to eq([])
    end
  end
end
